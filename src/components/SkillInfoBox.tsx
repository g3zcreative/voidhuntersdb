...
export function SkillInfoBox({ skill }: { skill: SkillData }) {
  ...
  // Parse effects object: expects keys like "lv2", "lv3" etc or flat array
  ...
  // Highlight numbers and special words in skill text
  function renderHighlighted(text: string) {
    // Match percentages, numbers, and capitalized special words
    const parts = text.split(/(\d+%|\d+(?:\.\d+)?|Stagger|Burn|Freeze|Poison|Bleed|Stun|Control|Silence|Slow|Weaken)/g);
    return parts.map((part, i) => {
      ...
      if (/^(Stagger|Burn|Freeze|Poison|Bleed|Stun|Control|Silence|Slow|Weaken)$/.test(part)) {
        return (
      ...
      <div className="p-4 pb-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-display text-lg font-bold tracking-tight text-foreground uppercase leading-tight">
            {skill.name}
            {maxLvl > 1 && (
              <span className="text-muted-foreground text-sm font-normal normal-case ml-1.5">
                (Lv.{maxLvl})
              </span>
            )}
          </h3>
          {skill.sort_order && (
            <p className="text-xs text-muted-foreground">{skill.sort_order}</p>
          )}
        </div>
      </div>
        ...
        <div className="px-4 pb-3 flex flex-wrap gap-1.5">
          {skill.type.split(/[,/]+/).map((t) => t.trim()).filter(Boolean).map((tag) => (
            <Badge
              key={tag}
              variant="outline"
              className="text-[10px] px-2 py-0.5 border-border text-muted-foreground font-medium"
            >
              {tag}
            </Badge>
          ))}
